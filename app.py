...
ICON_NAME = "icon.ico" # UPDATED: Matches your filename
...
# --- CRITICAL: RESOURCE PATH FOR ONEFILE EXE ---
...
class HabitModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if r == 0:
            ...
            if role == Qt.FontRole: f = QFont(); f.setBold(True); f.setPointSize(11); return f
            if role == Qt.TextAlignmentRole: return Qt.AlignCenter
        if r == 1:
            if role == Qt.DisplayRole: return self._dates[c].strftime("%d")
            if role == Qt.BackgroundRole: return QColor("#FFD700") if c == self._today_idx else QColor("#E5E7E9") 
            if role == Qt.FontRole: f = QFont(); f.setBold(True); return f
            if role == Qt.TextAlignmentRole: return Qt.AlignCenter
        habit_idx = r - 2
        ...
        if role == Qt.ToolTipRole: return f"{HABITS[habit_idx]} on {self._dates[c].strftime('%b %d')}"
        return None
...
class HabitApp(QWidget):
    ...
    def setup_ui(self):
        ...
        self.setStyleSheet("QWidget { background-color: #F0F2F5; font-family: 'Segoe UI', sans-serif; color: #000000; } QTableView { gridline-color: #E5E7E9; border: 1px solid #BDC3C7; }")
        layout = QVBoxLayout(self)
        ...
        btn_export.setStyleSheet("QPushButton { background-color: #3498DB; color: white; border-radius: 6px; padding: 8px 16px; font-size: 13px; font-weight: 600; border: none; } QPushButton:hover { background-color: #2980B9; } QPushButton::menu-indicator { image: none; }")
        
        menu = QMenu(self)
        menu.setStyleSheet("QMenu { background-color: white; border: 1px solid #BDC3C7; border-radius: 6px; padding: 5px 0px; } QMenu::item { padding: 8px 25px 8px 10px; font-size: 13px; color: #2C3E50; } QMenu::item:selected { background-color: #F4F6F8; color: #2980B9; } QMenu::icon { padding-left: 10px; }")
        ...
        header_layout.addWidget(title); header_layout.addStretch(); header_layout.addWidget(btn_export); header_layout.addSpacing(15); header_layout.addWidget(date_lbl)
        layout.addLayout(header_layout)

        kpi_layout = QHBoxLayout()
        self.card_today = KPICard("TODAY %"); self.card_streak = KPICard("BEST STREAK"); self.card_weekly = KPICard("WEEKLY AVG"); self.card_monthly = KPICard("MONTHLY AVG")
        ...
        self.table.horizontalHeader().setVisible(False); self.table.verticalHeader().setFixedWidth(100); self.table.verticalHeader().setDefaultSectionSize(40); self.table.setColumnWidth(0, 40)
        for i in range(DAYS): self.table.setColumnWidth(i, 35)
        self.table.setSelectionMode(QAbstractItemView.NoSelection); self.table.setFocusPolicy(Qt.NoFocus); self.table.clicked.connect(self.on_cell_clicked)
        layout.addWidget(self.table, stretch=2)

        self.fig = Figure(figsize=(8, 2), dpi=100)
        self.fig.patch.set_facecolor('#F0F2F5')
        self.canvas = FigureCanvasQTAgg(self.fig); self.canvas.setFixedHeight(220)
    ...
    def update_analytics(self):
        ...
        self.card_today.set_value(f"{self.today_pct}%"); self.card_streak.set_value(f"{self.best_streak} Days"); self.card_weekly.set_value(f"{self.weekly_avg}%"); self.card_monthly.set_value(f"{self.monthly_avg}%")
        daily_avg = [sum(self.habit_data[r][c] for r in range(len(HABITS))) / len(HABITS) * 100 for c in range(DAYS)]
        self.fig.clear(); ax = self.fig.add_subplot(111); ax.set_facecolor('#F0F2F5')
        ax.fill_between(range(DAYS), daily_avg, color='#3498DB', alpha=0.5); ax.plot(range(DAYS), daily_avg, color='#2980B9', linewidth=1.5)
        ax.set_title("Yearly Consistency Trend", fontsize=10, fontweight='bold', color='#2C3E50', pad=10); ax.set_ylim(0, 105); ax.set_xlim(0, DAYS); ax.axis('off'); self.canvas.draw()
    def export_csv(self):
        path, _ = QFileDialog.getSaveFileName(self, "Save CSV Report", f"Habit_Report_{YEAR}.csv", "CSV Files (*.csv)")
        if not path: return
        try:
            with open(path, "w", newline="") as f:
                writer = csv.writer(f); writer.writerow(["Date"] + HABITS + ["Daily Completion %"])
                for c, date in enumerate(self.dates):
                    row = [date.strftime("%Y-%m-%d")]
                    daily_sum = 0
                    for r in range(len(HABITS)):
                        val = self.habit_data[r][c]; row.append("Completed" if val == 1 else "Missed"); daily_sum += val
                    row.append(f"{int(daily_sum/len(HABITS)*100)}%"); writer.writerow(row)
    ...
    def export_pdf(self):
        ...
        try:
            ...
            c = canvas.Canvas(path, pagesize=letter); width, height = letter
            c.setFont("Helvetica-Bold", 24); c.drawString(50, height - 60, f"Habit Dashboard Report - {YEAR}")
            c.setFont("Helvetica", 12); c.drawString(50, height - 85, f"Generated on: {datetime.date.today().strftime('%B %d, %Y')}")
            y_kpi = height - 160; c.setStrokeColor(colors.lightgrey); c.setFillColor(colors.whitesmoke)
            ...
            for i in range(4):
                x_pos = 50 + (i * 130); c.rect(x_pos, y_kpi, 120, 60, fill=1); c.setFillColor(colors.black); c.setFont("Helvetica-Bold", 10); c.drawCentredString(x_pos + 60, y_kpi + 40, labels[i]); c.setFont("Helvetica-Bold", 18); c.drawCentredString(x_pos + 60, y_kpi + 15, values[i]); c.setFillColor(colors.whitesmoke)
            c.drawImage("temp_chart.png", 50, height - 400, width=500, height=180)
            y_table = height - 450; c.setFillColor(colors.black); c.setFont("Helvetica-Bold", 14); c.drawString(50, y_table, "Monthly Completion Rates"); y_row = y_table - 30; c.setFont("Helvetica", 10)
            c.drawString(50, y_row, "Month"); c.drawString(200, y_row, "Completion %"); y_row -= 20; c.line(50, y_row + 15, 300, y_row + 15)
            current_month = -1; month_total = 0; month_days = 0
            for i, date in enumerate(self.dates):
                if date.month != current_month:
                    if current_month != -1:
                        avg = int((month_total / (len(HABITS) * month_days)) * 100) if month_days > 0 else 0
                        c.drawString(50, y_row, datetime.date(YEAR, current_month, 1).strftime("%B")); c.drawString(200, y_row, f"{avg}%"); y_row -= 20
                    current_month = date.month; month_total = 0; month_days = 0
                month_total += sum(self.habit_data[r][i] for r in range(len(HABITS))); month_days += 1
            avg = int((month_total / (len(HABITS) * month_days)) * 100) if month_days > 0 else 0
            c.drawString(50, y_row, datetime.date(YEAR, current_month, 1).strftime("%B")); c.drawString(200, y_row, f"{avg}%"); c.save()
...
if __name__ == "__main__":
    # --- FIX: SET TASKBAR ICON ---
    # This separates your app from the generic Python process in the Taskbar
    myappid = 'mycompany.habit.tracker.1.0' 
    try:
    ...
    # --- FIX: ENSURE APP ICON IS LOADED FOR TASKBAR ---
    # We load it into the main app instance as well
