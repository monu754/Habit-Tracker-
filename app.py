...
THEME_LIGHT = {
    ...
    # UNDO BAR: Transparent BG, Dark Text
...
THEME_DARK = {
    ...
    # UNDO BAR: Transparent BG, White Text
...
class UndoBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.setContentsMargins(0, 0, 20, 0) # Right padding for contents
    ...
    def show_message(self, text="Item deleted", duration=4000, is_dark=False):
        ...
        text_color = theme["undo_text"]
        btn_color = theme["undo_btn"]
        
        self.setStyleSheet(f"""
            QWidget {{ background: transparent; }}
            QLabel {{ color: {text_color}; font-weight: bold; font-size: 14px; margin-right: 15px; }}
            QPushButton {{ color: {btn_color}; font-weight: 900; font-size: 14px; border: none; background: transparent; text-align: right; }}
        ...
        # Slide up animation (RIGHT SIDE)
        self.anim = QPropertyAnimation(self, b"pos")
        parent_rect = self.parent().rect()
        
        # X position: Width of parent - Width of widget - Margin (40px)
...
class HoverHeader(QHeaderView):
    ...
    def __init__(self, orientation, parent=None):
        ...
        self.setMouseTracking(True); self.hover_row = -1; self.setSectionsClickable(True)
...
class KPICard(QFrame):
    ...
    def set_value(self, text_val):
        try:
            val = int(''.join(filter(str.isdigit, str(text_val))))
        except: val = 0
    ...
    def _animate_step(self):
        if self._current_val < self._target_val:
            self._current_val += 1
        elif self._current_val > self._target_val:
            self._current_val -= 1
        else:
...
class HabitModel(QAbstractTableModel):
    ...
    def update_month_properties(self):
        ...
        if today.year == self._year and today.month == self._month:
            self.today_idx = today.day - 1

    def update_view(self, year, month, month_slice):
        self.layoutAboutToBeChanged.emit()
        self._year = year
        self._month = month
        self._month_data = month_slice
        self.update_month_properties()
    ...
    def headerData(self, section, orientation, role):
        ...
        if orientation == Qt.Vertical and role == Qt.FontRole and section >= 2:
    ...
    def toggle(self, index):
        ...
        self.dataToggled.emit(habit_idx, c)
...
class HabitApp(QWidget):
    def __init__(self):
        ...
        self.selected_habit_idx = None
        self._last_deleted_habit = None  
        
        self.chart_update_timer = QTimer()
        self.chart_update_timer.setSingleShot(True)
        self.chart_update_timer.timeout.connect(self.update_charts_data_only)
    ...
    def init_data(self):
        ...
        self.ensure_year_data(self.view_year)

    def ensure_year_data(self, year):
        str_year = str(year)
        days_in_year = 366 if calendar.isleap(year) else 365
        if str_year not in self.history_data:
            self.history_data[str_year] = [[0]*days_in_year for _ in self.habit_names]
        
        current_rows = len(self.history_data[str_year])
        target_rows = len(self.habit_names)
        if current_rows < target_rows:
            for _ in range(target_rows - current_rows):
                self.history_data[str_year].append([0]*days_in_year)
    ...
    def setup_ui(self):
        ...
        v_header.setDragEnabled(True)
        v_header.setDragDropMode(QAbstractItemView.InternalMove)
        v_header.sectionMoved.connect(self.on_habit_reorder)
    ...
    def resizeEvent(self, event):
        if self.undo_bar.isVisible():
            ...
            self.undo_bar.move(
                (self.width() - self.undo_bar.width()) - 40, # RIGHT ALIGN
                self.height() - 80
            )
        super().resizeEvent(event)

    def init_chart_setup(self):
        # Annual Setup
    ...
    def change_month(self, delta):
        new_month = self.view_month + delta
        new_year = self.view_year
    ...
    def refresh_habit_menu(self):
        ...
        def make_action(text, idx):
            ...
            action.triggered.connect(lambda: self.set_habit_view(idx, text))
            return action
        self.habit_menu.addAction(make_action("Global Overview", None))
        self.habit_menu.addSeparator()
        for i, name in enumerate(self.habit_names):
            self.habit_menu.addAction(make_action(name, i))

    def set_habit_view(self, idx, text):
        self.selected_habit_idx = idx
        self.btn_habit_filter.setText(text)
        self.trigger_full_update()

    def on_habit_reorder(self, logicalIndex, oldVisualIndex, newVisualIndex):
        if oldVisualIndex < 2 or newVisualIndex < 2: 
            self.table.verticalHeader().viewport().update()
            return 

        old_idx = oldVisualIndex - 2
        new_idx = newVisualIndex - 2

        self.habit_names.insert(new_idx, self.habit_names.pop(old_idx))
        self.habit_times.insert(new_idx, self.habit_times.pop(old_idx))
        
        for year in self.history_data:
            self.history_data[year].insert(new_idx, self.history_data[year].pop(old_idx))
    ...
    def apply_theme(self):
        ...
        self.btn_habit_filter.update_colors(theme['btn_filter_bg'], theme['btn_filter_text'])
        ...
        for c in [self.grid_container, self.chart_container]: 
            c.setStyleSheet(f"background: {theme['card']}; border: 1px solid {theme['border']}; border-radius: 12px;")
    ...
    def scroll_to_today_column(self):
        today = datetime.date.today()
        if today.year == self.view_year and today.month == self.view_month:
            col = today.day - 1
            self.table.scrollTo(self.model.index(0, col), QAbstractItemView.PositionAtCenter)
    ...
    def handle_header_menu(self, pos):
        ...
        if action == edit: 
            self.edit_habit_by_row(row)
        elif action == delete:
            reply = QMessageBox.question(self, 'Delete Habit', 
                                         f"Are you sure you want to delete '{self.habit_names[habit_idx]}'?\nThis cannot be fully undone once you close the app.",
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.Yes:
                self.delete_habit(habit_idx)
    ...
    def delete_habit(self, habit_idx):
        ...
        self._last_deleted_habit = {
            "index": habit_idx,
            "name": name,
            "time": self.habit_times[habit_idx],
            "history": {y: self.history_data[y][habit_idx] for y in self.history_data}
        }
        
        # If we are deleting the currently viewed habit, reset the view to global
        if self.selected_habit_idx == habit_idx:
            self.selected_habit_idx = None
            self.btn_habit_filter.setText("Global Overview")

        # Remove data
        self.habit_names.pop(habit_idx)
        self.habit_times.pop(habit_idx)
        for y in self.history_data: 
            self.history_data[y].pop(habit_idx)
        ...
        self.undo_bar.show_message(f"Deleted '{name}'", is_dark=self.is_dark_mode)

    def restore_last_deleted(self):
        if not self._last_deleted_habit: return
        
        data = self._last_deleted_habit
        idx = data["index"]
        
        if idx > len(self.habit_names): idx = len(self.habit_names)
        
        self.habit_names.insert(idx, data["name"])
        self.habit_times.insert(idx, data["time"])
        ...
        if d.exec_() == QDialog.Accepted:
            ...
            if n:
                ...
                for y in self.history_data:
                    ...
                    self.history_data[y].append([0]*days)
    ...
    def on_data_toggled(self, habit_idx, col_in_month):
        start_of_month_idx = datetime.date(self.view_year, self.view_month, 1).timetuple().tm_yday - 1
        global_idx = start_of_month_idx + col_in_month
        val = self.model._month_data[habit_idx][col_in_month]
        self.history_data[str(self.view_year)][habit_idx][global_idx] = val
        self.save_data()
        self.update_kpis()
        self.chart_update_timer.start(300)

    def save_data(self):
        data = { "names": self.habit_names, "times": self.habit_times, "history": self.history_data, "theme": self.is_dark_mode }
        ...
        if path:
            try:
                with open(path, "r") as f:
                    ...
                    self.is_dark_mode = d.get("theme", False)
                    raw = d.get("data", [])
                    if raw and isinstance(raw[0], list): self.history_data = {"2026": raw}
                    else: self.history_data = d.get("history", {})
                    self.ensure_year_data(self.view_year)
    ...
    def calculate_stats(self, habit_idx=None):
        ...
        if n == 0: return {}
        current_year_data = self.history_data[str(self.view_year)]
        days_in_year = len(current_year_data[0])
        today = datetime.date.today()
        if today.year == self.view_year: today_idx = today.timetuple().tm_yday - 1
        elif today.year > self.view_year: today_idx = days_in_year - 1 
        else: today_idx = -1
        ...
        if habit_idx is None:
            ...
            def get_avg(days):
                ...
                start = max(0, today_idx - days + 1)
                total = n * (today_idx - start + 1)
                done = sum(sum(current_year_data[r][c] for r in range(n)) for c in range(start, today_idx+1))
                return int((done/total)*100) if total > 0 else 0
        else:
            row_data = current_year_data[habit_idx]
            total_days = sum(row_data) 
            today_val = 100 if (0 <= today_idx < days_in_year and row_data[today_idx] == 1) else 0
            streak = 0; curr = 0
            for c in range(days_in_year):
            ...
            def get_avg(days):
                if today_idx < 0: return 0
                start = max(0, today_idx - days + 1)
                done = sum(row_data[c] for c in range(start, today_idx+1))
    ...
    def trigger_full_update(self):
        self.update_kpis()
        self.update_charts_data_only()

    def update_kpis(self):
        target_habit_idx = self.selected_habit_idx
        stats = self.calculate_stats(target_habit_idx)
        if not stats: return
        self.card_today.set_value(stats["today"]); self.card_streak.set_value(stats["streak"]); self.card_weekly.set_value(stats["weekly"]); self.card_monthly.set_value(stats["monthly"])
        self.card_total.set_value(stats["total"])

    def update_charts_data_only(self):
        target_habit_idx = self.selected_habit_idx
        n = len(self.habit_names); theme = THEME_DARK if self.is_dark_mode else THEME_LIGHT
        current_data = self.history_data[str(self.view_year)]; days_in_year = len(current_data[0])
        
        if target_habit_idx is not None:
            if target_habit_idx >= len(current_data):
                return # Safety
        ...
        # 1. Update Annual Line Chart
        self.line_annual.set_data(range(days_in_year), daily_avgs)
        self.line_annual.set_color(theme['chart_line'])
        ...
        monthly_data = {}
        for c in range(days_in_year):
            d_date = datetime.date(self.view_year, 1, 1) + datetime.timedelta(days=c)
            m = d_date.month
        ...
        for bar, h, lbl in zip(self.bars_monthly, month_avgs, self.bar_labels):
            bar.set_height(h)
            bar.set_color(theme['chart_bar'])
            
            if h > 1: 
                lbl.set_text(f"{int(h)}%")
                lbl.set_y(h + 2) 
                lbl.set_color(theme['text_primary'])
        ...
        self.ax_monthly.spines['bottom'].set_color(theme['border'])
        self.fig_monthly.patch.set_facecolor(theme['chart_bg']); self.ax_monthly.set_facecolor(theme['chart_bg'])
        
        month_labels = [calendar.month_abbr[m] for m in range(1, 13)]
        self.ax_monthly.set_xticks(range(12)); self.ax_monthly.set_xticklabels(month_labels)
        self.ax_monthly.set_ylim(0, 115)
    ...
    def export_csv(self):
        ...
        if path:
            ...
            with open(path, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["--- HABIT DATA ---"])
                writer.writerow(["Date"] + [f"{n}" for n in self.habit_names])
                start = datetime.date(self.view_year, 1, 1)
                for i in range(len(current_data[0])):
                    d = start + datetime.timedelta(days=i)
                    row = [d.strftime("%Y-%m-%d")] + ["Yes" if current_data[r][i] else "No" for r in range(len(self.habit_names))]
                    writer.writerow(row)
    ...
    def export_pdf(self):
        ...
        try:
            ...
            c.setFont("Helvetica", 12); y_text = h - 90
            c.drawString(50, y_text, f"• Report: {subtitle}")
            
            y_start = h - 120
            c.drawString(50, y_start, f"• Today's Completion: {stats.get('today', 'N/A')}")
            c.drawString(300, y_start, f"• Best Streak: {stats.get('streak', 'N/A')}")
            
            y_start -= 25
            c.drawString(50, y_start, f"• Weekly Average: {stats.get('weekly', 'N/A')}")
            c.drawString(300, y_start, f"• Monthly Average: {stats.get('monthly', 'N/A')}")
            
            y_start -= 25
            c.drawString(50, y_start, f"• Total Completions: {stats.get('total', 'N/A')}")
            
            c.drawImage("temp_annual.png", 50, h-400, width=500, height=200, preserveAspectRatio=True)
            c.drawImage("temp_monthly.png", 50, h-650, width=500, height=200, preserveAspectRatio=True)
            c.save()
